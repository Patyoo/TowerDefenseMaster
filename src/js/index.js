...
function shoot(delta){
    for(var i=0;i<projectiles.length;i++){
        if(projectiles[i].acquireEnemy(projectiles[i].target.x,projectiles[i].target.y,delta)==1){
            ...
            else animations.push(new Explosion(projectiles[i].x,projectiles[i].y,sizeTile,0));
...
function generateRadius(){
    radiusTowers.forEach(element => {
        element.drawRadius();
        if(element.radiusShownTime==element.turrentTick){
            element.turrentTick=0;
            radiusTowers.splice(radiusTowers.indexOf(element),1);
        }
     });
}
...
canvas.addEventListener('dblclick', function(evt) {
    ...
    if(mousePos.y>99){
        ...
        if(maps[mapChoice][(posYMap-100)/50][posXMap/50]>9 && money>=(searchedTower.price/2)){
            ...
            searchedTower.range+=(searchedTower.range/2);
...
canvas.addEventListener('click',function(evt){
    ...
    if(mousePos.y>99){
        var searchedTower=getTower(posXMap,posYMap);
        if(searchedTower!=undefined){
            var found=(radiusTowers[0]==searchedTower);
            if(found==0){
                if(radiusTowers.length) radiusTowers[0].turrentTick=0;
                radiusTowers=[];
                radiusTowers.push(searchedTower);
                }
            }
    }
    else{
